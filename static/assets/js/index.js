...
function initMap() {
  ...
  // Tạo marker và hiển thị tất cả các tàu
  displayAllShips();
  ...
  focusInput.forEach(item => item.addEventListener('change', () => {
    if (item.getAttribute('data-id') === 'all-ships') {
        displayAllShips();
    } else {
        let shipName = item.getAttribute('data-id');
        displaySingleShip(shipName);
    }
  }));

  setInterval(() => {
    if (markers.length == 1) {
        const markerInfo = markers[0].metadata;
        const shipName = markerInfo.shipId;
        displaySingleShip(shipName);
    } else {
        displayAllShips();
    }
...
function displayAllShips() {
  ...
  .then(data => {
      ...
      let centerPos = { lat: 13.678354, lng: 111.400118 };
      map.setCenter(centerPos);
      map.setZoom(6);

      ships.forEach((ship) => {
        const marker = new google.maps.Marker({
          position: { lat: parseFloat(ship.ViDo), lng: parseFloat(ship.KinhDo) },
          map: map,
          title: `Tàu ${ship.SoDangKy}`,
          icon: {
            url: '/static/assets/imgs/fishing-boat-resize.png',  // sau nên trả url về từ server
          },
          metadata: {
            shipId: ship.SoDangKy
          }
        });
        markers.push(marker);
        ...
        const infoWindow = new google.maps.InfoWindow({
            content: `
                <div class="p-3">
                    <h3 class="fs-5">Vị trí tàu cá ${ship.SoDangKy}</h3>
                    <p class="fs-6"><strong>Thông tin:</strong> </p>
                    <p class="fs-6">Tàu cá: ${ship.SoDangKy}</p>
                    <p class="fs-6">Chủ tàu: ${ship.ChuTau}</p>
                    <p class="fs-6">Thuyền trưởng: ${ship.ThuyenTruong}</p>
                    <p class="fs-6">Kinh độ: ${ship.KinhDo}</p>
                    <p class="fs-6">Vĩ độ: ${ship.ViDo}</p>
                    <p class="fs-6">Ngày cập nhật: ${ship.NgayCapNhat}</p>
                </div>
            `
        });
        ...
        focusInput.forEach(input => {
            ...
            if (SoDangKy == ship.SoDangKy) {
                input.setAttribute('data-lat', ship.ViDo);
                input.setAttribute('data-lng', ship.KinhDo);
            }
        });
      });
  })
...
function displaySingleShip(shipName) {
  // Xóa các marker cũ
  deleteMarkers();

  fetch(`/api/location/${shipName}/`)
  .then(response => {
    if (!response.ok) {
        throw new Error('Có lỗi khi lấy dữ liệu!!');
    } 
  ...
  .then(data => {
    ...
    let position = {lat: parseFloat(bundle.lat), lng: parseFloat(bundle.lng)};

    const marker = new google.maps.Marker({
        position: position,
        map: map,
        title: `Tàu ${bundle.SoDangKy}`,
        icon: {
            url: '/static/assets/imgs/fishing-boat-resize.png',  // sau nên trả url về từ server
        },
        metadata: {
            shipId: bundle.SoDangKy
        }
    });
    markers.push(marker);
    ...
    const infoWindow = new google.maps.InfoWindow({
        content: `
            <div class="p-3">
                <h3 class="fs-5">Vị trí tàu cá ${bundle.SoDangKy}</h3>
                <p class="fs-6"><strong>Thông tin:</strong> </p>
                <p class="fs-6">Tàu cá: ${bundle.SoDangKy}</p>
                <p class="fs-6">Chủ tàu: ${bundle.shipowner}</p>
                <p class="fs-6">Thuyền trưởng: ${bundle.captain}</p>
                <p class="fs-6">Kinh độ: ${bundle.lng}</p>
                <p class="fs-6">Vĩ độ: ${bundle.lat}</p>
                <p class="fs-6">Ngày cập nhật: ${bundle.date}</p>
            </div>
        `
    });
